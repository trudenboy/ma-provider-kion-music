"""KION Music authentication flows.
...
_DEVICE_CODE_PAGE_PATH = "/kion_music/device_code"
...
def _build_device_code_page(
    ...
    Kion's verification page does not pre-fill the code from query params,
...
<head>
    <meta charset="utf-8">
    <title>KION Music — Device Code</title>
...
<body>
    <div class="card" id="card">
        ...
        <a class="btn" href="{safe_url}" target="_blank" rel="noopener">Continue to Kion</a>
    </div>
    <script>
...
async def perform_device_auth(mass: MusicAssistant, session_id: str) -> tuple[str, str, str]:
    """Perform Kion OAuth Device Flow and return credential tokens.

    Asks Kion for a device code, presents it to the user via an intermediate
    ...
    try:
        ...
        raise LoginFailed(f"Kion device auth error: {err}") from err
...
async def perform_qr_auth(mass: MusicAssistant, session_id: str) -> tuple[str, str]:
    ...
    try:
        ...
        raise LoginFailed(f"Kion auth error: {err}") from err
...
async def refresh_music_token(x_token: SecretStr) -> SecretStr:
    ...
    try:
        ...
        raise ResourceTemporarilyUnavailable(
            f"Kion Passport temporarily unavailable: {err}"
        ) from err
    except YaPassportError as err:
...
async def validate_x_token(x_token: SecretStr) -> bool:
    """Return True if *x_token* is still accepted by Kion Passport."""
    try:
        async with PassportClient.create() as client:
