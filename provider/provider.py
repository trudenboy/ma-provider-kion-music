...
def _split_wave_mode(station_id: str) -> tuple[str, dict[str, str]]:
    ...
    to Kion; the part after is a key into WAVE_MODE_PRESETS.
    ...
    :return: Tuple of (base_station_id, settings_dict). The suffix, if
        ...
        Kion. ``settings_dict`` is the preset's settings when the suffix
...
class KionMusicProvider(MusicProvider):
    ...
    _wave_states: dict[str, _WaveState]  # Per-station state (incl. My Mix)
    ...
    async def handle_async_init(self) -> None:
        ...
        logging.getLogger("music_assistant.providers.kion_music").setLevel(self.logger.level)
        self._streaming = KionMusicStreamingManager(self)
        # Per-station wave state (incl. My Mix under ROTOR_STATION_MY_MIX).
    ...
    async def browse(  # noqa: PLR0911, PLR0915
        ...
        if subpath == MY_WAVE_PLAYLIST_ID:
            async with self._get_wave_state(ROTOR_STATION_MY_MIX).lock:
        ...
        if mode_preset is not None:
            if mode_preset not in WAVE_MODE_PRESETS:
                return []
            station_key = f"{ROTOR_STATION_MY_MIX}{WAVE_MODE_SEP}{mode_preset}"
        ...
        if preset_idx is not None:
            ...
            station_key = f"{ROTOR_STATION_MY_MIX}{WAVE_MODE_SEP}preset_{preset_idx}"
    ...
    async def _browse_my_wave(
        ...
        """Browse My Mix tracks (must be called under the My Mix state lock).
        ...
        """
        wave = self._get_wave_state(ROTOR_STATION_MY_MIX)
        ...
        for _ in range(max_batches):
            ...
            yandex_tracks, batch_id = await self._fetch_rotor_session_batch(
                wave, ROTOR_STATION_MY_MIX
            )
            if batch_id:
                last_batch_id = batch_id
            if not wave.radio_started_sent and yandex_tracks:
                sent = await self._send_wave_feedback(wave, ROTOR_STATION_MY_MIX, "radioStarted")
                if sent:
    ...
    def _parse_my_wave_track(
        ...
        seen_ids: set[str],
        *,
        station_key: str = ROTOR_STATION_MY_MIX,
    ) -> Track | None:
        ...
        Callers using shared state must hold the My Mix state lock.
        ...
        :param station_key: Station key to embed in the composite item_id.
            Defaults to the plain My Mix station.
    ...
    async def _send_wave_feedback(
        ...
        *,
        ...
        :param track_id: Kion track ID the event refers to.
    ...
    async def _prefetch_rotor_session(self, station_key: str) -> None:
        ...
        we already have Kion-curated wave tracks sitting in
    ...
    async def _fetch_rotor_session_batch(
        ...
        base station ID (before "#") is what actually goes to Kion.
        ...
        :return: Tuple of (list of kion tracks, batch_id or None).
        """
    ...
    async def _browse_wave_station(
        ...
        async with state.lock:
            ...
            # verified against the live Kion API. Reuse the session helper so
    ...
    async def search(
        ...
        # Map MediaType to Kion API search type. AUDIOBOOK has no dedicated
        # Kion type — it maps to "album" and is filtered by classify_album below.
        type_mapping = {
        ...
        # classify_album. Kion-returned podcast albums are handled separately
            ...
            for _ in range(MY_WAVE_BATCH_SIZE):
                ...
                if not wave.radio_started_sent and yandex_tracks:
                    sent = await self._send_wave_feedback(
                        wave, ROTOR_STATION_MY_MIX, "radioStarted"
                    )
                    if sent:
                        wave.radio_started_sent = True
    ...
    async def _get_liked_tracks_playlist_tracks(self, page: int) -> list[Track]:
        ...
        for i in range(0, len(track_ids), batch_size):
            ...
            # a 500-track hydration doesn't look like a bot to Kion's
    ...
    async def _get_my_wave_recommendations(self) -> RecommendationFolder | None:
        ...
        Shares the same `_WaveState(ROTOR_STATION_MY_MIX)` with browse and
        ...
        for gen_playlist in feed.generated_playlists:
            if gen_playlist.data and gen_playlist.ready:
                try:
                    ...
                    # Premiere, Missed Likes) as dynamic — Kion regenerates them
    ...
    async def library_add(self, item: MediaItemType) -> bool:
        ...
        the user adds a My Mix track to favourites during playback), also
    ...
    async def get_rotor_station_tracks(
        ...
        (`KionMusicProviderLike.get_rotor_station_tracks`). The
    ...
    async def on_played(
        ...
        - Audiobooks: persist chapter progress via play_audio so Kion's
        ...
        known channel Kion writes into ``/handlers/music-history`` is a
    ...
    async def on_streamed(self, streamdetails: StreamDetails) -> None:
        """Report stream completion to Kion.

        - Audiobooks: a final ``play_audio`` with the absolute stream
          position so the last listening point is preserved across Kion
        ...
        - Wave tracks (composite item_id carries a station suffix): a rotor
          ...
          so Kion can improve recommendations.
        """
        data = streamdetails.data if isinstance(streamdetails.data, dict) else None
    ...
    def _audiobook_progress_point(
        ...
        two invariants Kion cares about and that ``_resolve_audiobook_seek``
        alone doesn't guarantee:

        - At/beyond end-of-book, map to end of the last chapter (not start),
          so Kion's resume point doesn't rewind to the start of the final
    ...
    async def _report_audiobook_progress(self, audiobook_id: str, position_sec: int) -> None:
        """Push current listening position of an audiobook to Kion.

        Resolves the playing chapter + offset from the cached chapter map, then
        calls play_audio so Kion persists the position for cross-client resume.
