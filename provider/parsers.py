...
def classify_album(album_obj: YandexAlbum) -> AlbumKind:
    """Classify a Kion album as music / podcast / audiobook.
    ...
    field because Kion tags audiobooks with ``meta_type="podcast"`` *and*
    ``type="audiobook"`` — empirically observed in production libraries.
    Values are not documented in the kion_music SDK.
...
def detect_description_language(text: str | None) -> Literal["ru"] | None:
    ...
    KION Music's API does not expose the language of artist / playlist /
...
def _album_cover_images(
    provider: KionMusicProvider, album_obj: YandexAlbum
...
def parse_playlist(
    ...
    *,
    ...
    :param is_dynamic: Mark the playlist as dynamic so Music Assistant does
        not long-cache its content. Kion regenerates "Playlist of the Day",
...
def parse_podcast(provider: KionMusicProvider, album_obj: YandexAlbum) -> Podcast:
    """Parse Kion album (meta_type=podcast) to MA Podcast model.

    :param provider: The KION Music provider instance.
    :param album_obj: Kion album object classified as a podcast.
    :return: Music Assistant Podcast model.
    """
    if album_obj.id is None:
        raise InvalidDataError("Kion podcast missing id")
...
def parse_podcast_episode(
    ...
    """Parse Kion track (episode of a podcast album) to MA PodcastEpisode.
    ...
    """
    if track_obj.id is None:
        raise InvalidDataError("Kion podcast episode missing id")
...
def parse_audiobook(provider: KionMusicProvider, album_obj: YandexAlbum) -> Audiobook:
    """Parse Kion album (meta_type=audiobook) to MA Audiobook model.

    :param provider: The KION Music provider instance.
    :param album_obj: Kion album object classified as an audiobook.
    ...
    """
    if album_obj.id is None:
        raise InvalidDataError("Kion audiobook missing id")
    name, _ = parse_title_and_version(
