"""Kion Passport QR authentication flow.

Adapted from AlexxIT/YandexStation (MIT license) and
trudenboy/ma-provider-kion-station session.py.
...
# Kion Passport OAuth credentials (public, from KION Music Android app)
PASSPORT_CLIENT_ID = "c0ebe342af7d48fbbbfcf2d2eedb8f9e"
PASSPORT_CLIENT_SECRET = "ad0a908f0aa341a182a37ecd75bc319e"

# KION Music OAuth credentials (from kion-music-api)
...
# Mobile User-Agent required for Kion Passport to return CSRF token in HTML
...
class YandexQRAuth:
    """Kion Passport QR authentication.

    Uses a dedicated aiohttp session with its own cookie jar
    to avoid leaking Kion cookies into the shared MA session.
    """
    ...
    async def _post_json(
        ...
        try:
            async with self._session.post(url, data=data, headers=headers) as resp:
                if resp.status != 200:
                    raise LoginFailed(f"Kion returned HTTP {resp.status} for {resp.url.path}")
            ...
            raise LoginFailed(f"Network error during Kion auth: {type(err).__name__}") from err
    ...
    async def get_qr(self) -> tuple[str, str, str]:
        ...
        try:
            async with self._session.get(f"{PASSPORT_URL}/am?app_platform=android") as resp:
                if resp.status != 200:
                    raise LoginFailed(f"Kion Passport returned HTTP {resp.status}")
                html = await resp.text()
        except aiohttp.ClientError as err:
            raise LoginFailed(
                f"Network error reaching Kion Passport: {type(err).__name__}"
            ) from err

        # Try multiple patterns — Kion serves different page formats
        ...
        if not csrf_token_value:
            ...
            raise LoginFailed("Failed to obtain CSRF token from Kion Passport")
        ...
        if not track_id_value:
            raise LoginFailed("Kion Passport response missing track_id")
    ...
    async def get_x_token(self) -> str:
        ...
        if not filtered:
            raise LoginFailed("No Kion session cookies found after QR auth")
        ...
        data = await self._post_json(
            ...
            headers={
                "Ya-Client-Host": "passport.kion.ru",
