...
description: Как настроить визуальную презентацию и SEO для репозитория KION Music
---

Этот документ описывает, как поддерживается визуальное представление репозитория **KION Music** на GitHub и в поиске.
...
- **Badge "Music Assistant"** — динамический shields.io endpoint badge: `https://trudenboy.github.io/ma-provider-tools/badges/kion_music.json`. Cron `update-ma-version-badges.yml` обновляет JSON каждые 4 часа и показывает, в какие каналы MA (stable / beta / dev) включена текущая версия провайдера.
