...
# Dev environment setup for the kion_music provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/kion_music"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: kion_music → $TARGET"
  ...
  echo "Provider kion_music linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
